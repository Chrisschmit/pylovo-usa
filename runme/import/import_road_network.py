...
Usage:
    python -m runme.import.extract_road_network --plot
"""
import argparse
import time
...
from src.import_data.plotting.plot_road_network import visualize_road_network
    ...
    try:
        ...
        if plot:
            visualize_road_network(
                road_network_results['geojson_file'], boundary_gdf)

        logger.info("Road Network Extraction completed successfully.")

    except ValueError as ve:
        logger.error(
            "Configuration or validation error: %s",
            ve,
            exc_info=True)
...
def main():
    ...
    extract_road_network(
        boundary_file_path=args.boundary_file_path,
        plot=args.plot)
