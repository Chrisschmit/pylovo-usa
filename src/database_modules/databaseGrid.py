...
class GridMixin:
    ...
    def install_consumer_cables(self, plz: int, bcid: int, kcid: int, branch_deviation: float, branch_node_list: list,
        ...
        for vertice in branch_consumer_list:  # TODO: looping for duplicate vertices
            ...
            self.insert_lines(geom=line_geodata, plz=plz, bcid=bcid, kcid=kcid, line_name=f"Line to {end_vid}",
                              std_type=cable,
    ...
    def create_line_node_to_node(self, plz: int, kcid: int, bcid: int, branch_node_list: list, branch_deviation: float,
        ...
        for i in range(len(branch_node_list) - 1):
            ...
            if branch_node_list[i + 1] not in node_path_list:  # if next node of branch node list not in node path list
                ...
                node_path_list = self.get_path_to_bus(branch_node_list[i], branch_node_list[
    ...
    def create_line_start_to_lv_bus(self, plz: int, bcid: int, kcid: int, branch_start_node: int,
        ...
        self.insert_lines(geom=line_geodata, plz=plz, bcid=bcid, kcid=kcid, line_name=f"Line to {branch_start_node}",
                          std_type=cable, from_bus=pp.get_element_index(net, "bus", "LVbus 1"),
    ...
    def get_cables(self, anw: tuple) -> pd.DataFrame:
        query = """SELECT name, max_i_a, r_mohm_per_km, x_mohm_per_km, z_mohm_per_km, cost_eur
                   FROM equipment_data
                   WHERE typ = 'Cable'
                     AND application_area IN %(a)s
                   ORDER BY max_i_a DESC; """
    ...
    def position_brownfield_transformers(self, plz: int, kcid: int, transformer_list: list) -> None:
        ...
        for transformer_id in transformer_list:
            ...
            self.update_building_cluster(transformer_id, pre_result_dict[transformer_id], building_cluster_count, kcid,
    ...
    def insert_transformers(self, plz: int) -> None:
        ...
        insert_query = """
                       ...
                       WHERE ST_Within(t.geom, (SELECT geom
                                                ...
                                                WHERE postcode_result_plz = %(p)s AND version_id = %(v)s)); --IS FALSE;
    ...
    def get_grid_versions_with_plz(self, plz: int) -> list[tuple]:
        query = ("""SELECT DISTINCT version_id
                    FROM grid_result
                    WHERE plz = %(p)s""")
    ...
    def get_grids_from_plz(self, plz: int) -> pd.DataFrame:
        grids_query = """SELECT version_id, plz, kcid, bcid, grid
                         FROM grids_result
                         WHERE plz = %(p)s"""
        params = {"p": plz}
