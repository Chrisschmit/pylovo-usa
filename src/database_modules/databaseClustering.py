...
class ClusteringMixin:
    ...
    def get_clustering_parameters_for_plz_list(self, plz_tuple: tuple) -> pd.DataFrame:
        ...
        query = """
                WITH plz_table(plz) AS (VALUES (%(p)s)),
                     clustering AS (SELECT version_id, plz, kcid, bcid, cp.*
                                    FROM clustering_parameters cp
    ...
    def create_bcid_for_kcid(self, plz: int, kcid: int) -> None:
        ...
        while True:
            ...
            invalid_cluster_dict, cluster_dict, _ = self.try_clustering(Z, cluster_amount, new_localid2vid, buildings,
