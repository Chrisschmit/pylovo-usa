...
class ParameterCalculator:
    ...
    def calc_parameters_per_regional_identifier(self, regional_identifier):
        ...
        if grid_analysed:
            self.dbc.logger.info(f"Grid for the postcode area {regional_identifier} has already been analyzed.")
            return

        try:
            self.dbc.logger.info("Start basic result analysis")
            self.analyse_basic_parameters_per_regional_identifier(regional_identifier)
            ...
            self.analyse_trafo_parameters_per_regional_identifier(regional_identifier)
            self.dbc.logger.info("Result analysis finished")
            self.dbc.conn.commit()
        except Exception as e:
            self.dbc.logger.error(f"Error during analysis for regional_identifier {regional_identifier}: {e}")
            self.dbc.logger.info(f"Skipped regional_identifier {regional_identifier} due to analysis error.")
    ...
    def calc_parameters_per_grid(self, regional_identifier: int) -> None:
        ...
        parameter_count = self.dbc.count_clustering_parameters(regional_identifier=regional_identifier)
        if parameter_count > 0:
            print(
                f"The parameters for the grids of postcode area {regional_identifier} and version {VERSION_ID} "
                f"have already been calculated."
            )
            return

        cluster_list = self.dbc.get_list_from_regional_identifier(regional_identifier)
        for kcid, bcid in cluster_list:
            print(bcid, kcid)
            self.calc_grid_parameters(regional_identifier, bcid, kcid)

    def calc_grid_parameters(self, regional_identifier: int, bcid: int, kcid: int) -> None:
    ...
    def compute_parameters(self, net: pp.pandapowerNet) -> None:
        ...
        ) = self.calc_resistance(net, G)

        self.vsw_per_branch = self.resistance / self.no_branches

    def get_simultaneous_peak_load(self) -> float:
        data_list, _, _ = self.dbc.read_per_trafo_dict(self.regional_identifier)
    ...
    def calc_resistance(
        self, pandapower_net: pp.pandapowerNet, networkx_graph: nx.Graph
        ...
        df_vsw = (
            df_vsw.to_frame()
            .reset_index()
            .rename(columns={"bus": "house_connection", "max_p_mw": "household_equivalents"})
        )
        ...
        for index, row in df_vsw.iterrows():
            df_vsw.at[index, "path"] = nx.shortest_path(
                networkx_graph, source=root, target=df_vsw.at[index, "house_connection"]
            )
    ...
    def calculate_line_with_sim_factor(self, pandapower_net, networkx_graph) -> pd.DataFrame:
        ...
        net_line_with_sim_factor.drop(
            ["c_nf_per_km", "g_us_per_km", "max_i_ka", "df", "type", "in_service"], axis=1, inplace=True
        )
        ...
        level1 = pd.merge(
            left=pandapower_net.load, left_on="bus", right=pandapower_net.bus, right_on=pandapower_net.bus.index
        )
        ...
        load_count_cat = pd.merge(
            left=load_count, left_on="zone_x", right=df_sim_factor_definitions, right_on="description"
        )

        load_count_cat = load_count_cat.assign(
            sim_factor_level1=lambda x: utils.oneSimultaneousLoad(
                installed_power=1, load_count=x["count"], sim_factor=x["sim_factor"]
            )
        )
        ...
        for index, row in df_connection_bus.iterrows():
            ...
            load_public = utils.oneSimultaneousLoad(
                ...
                load_count=net_line_with_sim_factor.at[upstream_index[0], "no_public"],
                sim_factor=SIM_FACTOR["Public"],
            )
            ...
            net_line_with_sim_factor.at[upstream_index[0], "sim_load"] = (
                load_commercial + load_public + load_residential
            )

            peak_load_all_consumer_types = (
                net_line_with_sim_factor.at[upstream_index[0], "load_commercial_mw"]
            ...
            if peak_load_all_consumer_types == 0:
                net_line_with_sim_factor.at[
                    ...
                    upstream_index[0], "sim_factor_cumulated"
                ] = 0
            else:
    ...
    def _run_analysis_in_parallel(self, regional_identifier: int, worker_function):
        """
        ...
        with ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(worker_function, regional_identifier, kcid, bcid): (kcid, bcid)
                for kcid, bcid in cluster_list
            }
    ...
    @staticmethod
    def _process_basic_parameters(regional_identifier: int, kcid: int, bcid: int) -> list:
        ...
        for row in net.trafo[["sn_mva"]].itertuples():
            ...
            results.append(
                {"capacity": capacity, "load_count": load_count, "bus_count": bus_count, "cable_length": cable_length}
            )
        return results

    def analyse_basic_parameters_per_regional_identifier(self, regional_identifier: int):
        self.dbc.logger.debug("start basic parameter counting")
        results = self._run_analysis_in_parallel(regional_identifier, self._process_basic_parameters)
    ...
    @staticmethod
    def _process_cables(regional_identifier: int, kcid: int, bcid: int) -> dict:
        ...
        for cable_type in cable_types:
            ...
            cable_length_dict[cable_type] = (
                cable_df.loc[type_mask, "parallel"] * cable_df.loc[type_mask, "length_km"]
            ).sum()
    ...
    def analyse_cables_per_regional_identifier(self, regional_identifier: int):
        ...
        results = self._run_analysis_in_parallel(regional_identifier, self._process_cables)
    ...
    @staticmethod
    def _process_trafo_parameters(regional_identifier: int, kcid: int, bcid: int) -> dict:
        ...
        trafo_distances_to_buses = (
            top.calc_distance_to_bus(net, net.trafo["lv_bus"].iloc[0], weight="weight", respect_switches=False)
            .loc[load_buses]
            .tolist()
        )
        ...
        for building_type in building_types:
            ...
            if house_num > 0:
                ...
                sim_peak_load += utils.oneSimultaneousLoad(
                    installed_power=sum_load, load_count=house_num, sim_factor=SIM_FACTOR[building_type]
                )

        avg_distance = (sum(trafo_distances_to_buses) / len(trafo_distances_to_buses)) * 1e3
    ...
    def analyse_trafo_parameters_per_regional_identifier(self, regional_identifier: int):
        self.dbc.logger.debug("start per trafo analysis")
        results = self._run_analysis_in_parallel(regional_identifier, self._process_trafo_parameters)
        ...
        self.dbc.insert_trafo_parameters(
            regional_identifier, trafo_load_string, trafo_max_distance_string, trafo_avg_distance_string
        )
