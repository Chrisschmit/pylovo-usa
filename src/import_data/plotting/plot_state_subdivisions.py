...
def plot_subdivisions(csv_path: Path, show_labels: bool = True):
    ...
    if 'geom' not in df.columns:
        logger.error(
            "CSV file must contain a 'geom' column with WKB hex strings.")
        return
    ...
    logger.info(
        f"Successfully created GeoDataFrame with {
            len(gdf)} subdivisions.")
    ...
    if show_labels:
        ...
        for idx, row in gdf_web_mercator.iterrows():
            # Use representative_point to ensure the label is within the
            # polygon
    ...
    # 6. Save the plot
    output_filename = csv_path.parent.parent / \
        f"{state_name}_subdivisions_map.png"
