...
def plot_pie_of_trafo_cables(regional_identifier):
    """
    Plots a pie chart of the trafos and cable types for a regional_identifier
    """
    ...
    cable_dict = dbc_client.read_cable_dict(regional_identifier)
    axs[1].pie(cable_dict.values(), labels=cable_dict.keys(), autopct="%.1f%%")
    axs[1].set_title("Installed Cable Length", fontsize=14)
    plt.show()


def plot_hist_trafos(regional_identifier):
    """
    plots histogram of trafo sizes in regional_identifier
    """
    gg = GridGenerator(regional_identifier=regional_identifier)
    dbc_client = gg.dbc
    data_list, data_labels, trafo_dict = dbc_client.read_per_trafo_dict(
...
def plot_boxplot_regional_identifier(regional_identifier):
    """
...
def plot_cable_length_of_types(regional_identifier):
    """
...
def get_trafo_dicts(regional_identifier):
    """
    Retrieve load count, bus count and cable lenth per type for a regional_identifier
    """
    gg = GridGenerator(regional_identifier=regional_identifier)
    dbc_client = gg.dbc
    cluster_list = dbc_client.get_list_from_regional_identifier(
...
def plot_trafo_on_map(regional_identifier, save_plots: bool = False) -> None:
    """
    ...
    if save_plots:
        savepath_folder = Path(
            RESULT_DIR, "figures", f"version_{VERSION_ID}", regional_identifier
        )
